...
import MapaUSPG from './components/MapaUSPG'
          ...
          <div className="adm-modal">
            ...
            <div className="adm-modal-body">
              <p style={{ marginBottom: '12px', fontSize: '14px' }}>
                La reserva <strong>&ldquo;{conflictoModal.reservaPendiente.titulo}&rdquo;</strong> se traslapa con:
              </p>
...
export default function AdministracionDashboard({ initialData, userRole = 'ADMIN' }) {
  ...
  // Handler del mapa: al seleccionar un salón y pedir reserva, abre el modal preseleccionado
  const handleMapaReserva = (espacio) => {
    setShowReservaModal(true)
  }

  const handleMapaReporte = (espacioId) => {
    setEspacioPreseleccionado(espacioId || null)
    setShowReporteModal(true)
  }

  const allTabs = [
    { id: 'mapa',          label: 'Mapa',          icon: 'fa-map' },
  ...
  const tabs = isStudent
    ? allTabs.filter((t) => ['mapa', 'mantenimiento'].includes(t.id))
    : allTabs

  const statItems = [
    { id: 'espacios',      label: 'Total Espacios',    value: stats.totalEspacios },
    { id: 'mapa',          label: 'Disponibles',        value: stats.espaciosDisponibles },
        ...
        <div className="dashboard-card" style={{ padding: '20px' }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="adm-section-title" style={{ margin: 0 }}>
              <i className="fa fa-map" /> Plano de Salones — USPG
            </h2>
            <p style={{ fontSize: '12px', opacity: 0.55 }}>
              Haz clic en un salón para ver su estado y opciones
            </p>
          </div>
          <MapaUSPG
            espacios={espacios}
            onSeleccionar={() => {}}
            onNuevaReserva={handleMapaReserva}
            onReporte={handleMapaReporte}
          />
        </div>
      )}
