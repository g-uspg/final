  ...
  try {
    ...
    await prisma.reservaEspacio.createMany({
      data: ocurrencias.map((oc) => ({
        ...
        diasRecurrencia: Array.isArray(data.diasRecurrencia) ? data.diasRecurrencia.join(',') : (data.diasRecurrencia || null),
        grupoRecurrenciaId: data.recurrente ? grupoId : null,
        fechaFinRecurrencia: data.recurrente && data.fechaFinRecurrencia ? new Date(data.fechaFinRecurrencia) : null,
